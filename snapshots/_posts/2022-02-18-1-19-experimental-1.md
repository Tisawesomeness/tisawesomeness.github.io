...
Shriekers have a 10 second cooldown, and will not shriek if a warden intersects a 48x48x48 bounding box centered on the shrieker.

If a shrieker is activated when a player's warning level is at 3, then it will try to spawn a warden. Warden spawning uses the same code as [iron golem spawning](https://minecraft.fandom.com/wiki/Iron_Golem#Villages) with some changes. Up to 20 attempts are made to spawn a warden within a 11×13×11 box centered on the shrieker. **Wardens require a a light level of 11 or lower to spawn.**
