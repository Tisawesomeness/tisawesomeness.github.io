...
**The game no longer separates the raid countdown into three spawning phases.** Instead, each 20-tick interval has different radius and bias values:
...
As an example (assuming the raid center is at (0, 0)), if the raid countdown is within 20-39 ticks, the radius is 0.64 and the bias is between -2 and 0. Since the selected coordinate within the 0.64 block ring is rounded down to the nearest block, the possible coordinates are a 2x2 square: (0, 0), (-1, 0), (0, -1), and (-1, -1). Accounting for the -2 to 0 bias, mobs can spawn in a 4x4 square with the raid center in the +X+Z corner.
...
```java
// raidCooldownTicks decrements from 300 to 0
