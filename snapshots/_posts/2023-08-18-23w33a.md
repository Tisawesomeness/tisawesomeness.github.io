...
Chorus flower and spore blossom were added to the `#flowers` block/item tag, fixing {::bug:MC-214126}.

If an item is unstackable due to renaming in an anvil ({::bug:MC-125046}) or using in a grindstone ({::bug:MC-181109}), renaming it in an anvil again will remove the `{RepairCost:0}` NBT and allow the item to be stackable.
