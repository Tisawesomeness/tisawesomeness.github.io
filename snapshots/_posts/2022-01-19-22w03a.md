...
Removed powder snow from the `#goat_spawnable_on` block tag to prevent goats from spawning on powder snow ({::bug:MC-241152}).

Added snow block and powder snow to the `#azalea_root_replaceable` block tag so that rooted dirt from azalea trees can replace those blocks ({::bug:MC-242010}).
