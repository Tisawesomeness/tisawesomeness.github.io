...
In swamps and mangrove swamps, the skeleton spawn rate was reduced from 100 to 50, and the bogged spawn weight was reduced from 50 to 30, fixing {::bug:MC-268598}.

## Misc

If the `doImmediateRespawn` gamerule is true, players with the oozing, infested, or wind charged status effects will activate those effects immediately on death, fixing {::bug:MC-269951}.

The `fraction` enchnatment value provider will return 0 if the denominator is 0, fixing {::bug:MC-271716}.
