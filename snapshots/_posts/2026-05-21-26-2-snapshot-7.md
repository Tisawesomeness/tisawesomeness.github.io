...
Worlds published online require all chat messages and chat commands to be signed and reportable, equivalent to `enforce-secure-profile: true` on a server. Mods can change this.
...
A guest declines a host's invite by sending an `INVITE_DECLINED` signaling message:
...
A guest accepts a host's invite by sending a join request.

### Join Requests

A guest sends a `JOIN_REQUEST` when accepting a host's invite or when the guest clicks on the "request to join" button in the friends list.
...
Join requests and responses are sent as signaling messages:

- `type`: One of `JOIN_REQUEST`, `JOIN_ACCEPTED`, or `JOIN_REJECTED`
- `sessionId`: The same `Session-Id` sent to the Signaling API
...
- Worlds published online require chat signatures.
- Mojang technically has the capability to read and/or modify P2P traffic since they run the signaling channel, so "Mojang **cannot** read its content" was removed.
