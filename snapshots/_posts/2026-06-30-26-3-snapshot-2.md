...
Projectiles can no longer break blocks in adventure mode (fix for {::bug:MC-309081}) unless the item that shot the projectile's `can_break` component allows breaking that block.

### Block State Providers

The `copy_properties_provider` block state provider has a field named `source_block_state_provider`, not `block_state_provider`.
...
- Correct official changelog mistake for `copy_properties_provider`
