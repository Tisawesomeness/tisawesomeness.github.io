...
Mobs can now perform smash attacks with the mace, fixing {::bug:MC-269428}. The attack does the same damage and knockback as when a player performs a smash attack. **The mob does not reset its momentum or accumulated fall distance**, so the mob will take fall damage from the full height of the drop.

Mobs used to search for attackable targets within a 4 block vertical range of the mob's hitbox. The vertical search range was changed to `follow_range` blocks, fixing {::bug:MC-121013}.
...
Salmon variants currently have the {::bug:same hitbox size:MC-275222}.
...
Breezes now remember mobs for **5 seconds** after line of sight is lost, fixing {::bug:MC-273076}.
...
Players stop "using" items (such as holding a shield or eating food) when they switch dimensions, fixing {::bug:MC-86252} and {::bug:MC-274372}.

An entity cannot go through the end portal if any passenger is a player who has not seen the credits, fixing {::bug:MC-272603}.
...
Players can no longer fill cauldrons with lava or powder snow if the block above is water or a waterlogged block, fixing {::bug:MC-203748} and {::bug:MC-240948}. Players can still place water on top of lava or powder snow cauldrons and push cauldrons underwater with pistons.

Experience caps at 2147483647 levels, fixing {::bug:MC-3266}.
