...
This snapshot adds goats, a darker splash screen, a new mob cap, and the light block! See the [changelog](https://www.minecraft.net/en-us/article/minecraft-snapshot-21w13a).
...
- If the jump velocity exceeds 1.5 (probably blocks/sec, not sure)
...
Once a goat finds a jump, it **prepares for 40 ticks** (2 seconds) before jumping. Then, it sets a  **`600-1200` tick cooldown** (30-60 seconds, uniform random, both ends inclusive) between the next jump.
...
# Underwater Cave Mob Category

**Glow Squids and Axolotls** now count towards the `underground_water_creature` mob cap instead of the old water creature cap. Both caps hold **5 mobs** in singleplayer.
