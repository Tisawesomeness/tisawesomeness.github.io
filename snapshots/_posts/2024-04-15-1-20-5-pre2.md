...
The `generic.scale` attribute now has no effect on the Ender Dragon, fixing {::bug:MC-267414}.
...
The `table_bonus` loot item condition now requires at least one item in the `chances` list, fixing {::bug:MC-269472}.
