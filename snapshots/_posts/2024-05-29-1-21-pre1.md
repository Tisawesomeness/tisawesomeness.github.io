...
Slimes now use the same attack range calculations as other mobs, fixing {::bug:MC-272079}.
...
When a player is launched by a non-Breeze wind charge or the Wind Burst enchantment, fall damage reduction will not be reset within 2 seconds of the launch by touching a ladder, using the Elytra, or using a trident. The `current_impulse_context_reset_grace_time` int field in the player's NBT stores the 40-tick countdown. Fixes {::bug:MC-271971}.
