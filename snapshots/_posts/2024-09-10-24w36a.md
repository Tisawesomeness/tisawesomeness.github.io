...
A TNT minecart now only explode if it is moving horizontally at 0.1 blocks/tick or faster, meaning minecarts will not explode if they are completely stopped by a block. Fixes {::bug:MC-275622}

Moving entities now only check up to the first **16 blocks** along the movement path for collisions, fixing {::bug:MC-275437}.
...
Some properties mentioned in {::bug:MC-88967} are not transferred in any conversion:
...
- `CanBreakDoors` due to {::bug:MC-276611}
- `Fire` due to {::bug:MC-276612}
- `Gossips`
- `UUID`
- `VillagerData`
- `Xp`

Despite the fix in {::bug:MC-270842}, scoreboard objectives are not transferred.

Some properties mentioned in {::bug:MC-88967} are transferred in some conversions but not others:
