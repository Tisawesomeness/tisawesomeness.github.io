...
When Bad Omen is converted to Raid Omen, the player's position at that moment is saved as `raid_omen_position` in the player's NBT data. The raid will be started from that position. Converting Bad Omen to Raid Omen again will update the Raid Omen position, and update the potion effect timer using normal potion mechanics.
